          ...
          <div className="mt-10 grid gap-10 lg:grid-cols-[1fr_380px]">
            <ul className="space-y-4">
              {cart.map(({ product, qty }) => (
                <li key={product.id} className="grid grid-cols-[100px_1fr_auto] gap-5 rounded-lg border border-border bg-card p-4 md:grid-cols-[120px_1fr_auto]">
                  <Link to="/product/$id" params={{ id: product.id }}>
                    <img src={product.image} alt={product.name} className="aspect-[4/5] w-full rounded-md object-cover" />
                  </Link>
